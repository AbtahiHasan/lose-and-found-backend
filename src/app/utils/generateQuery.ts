...
const generateQuery = async <T extends Document>(
  query: TQuery,
  model: Model<T>,
) => {
  const filter: any = {};
  // check which value exits in query and make filterable object
  if (query.minPrice !== undefined)
    filter.price = { $gte: parseFloat(query.minPrice) };
  if (query.maxPrice !== undefined)
    filter.price = { ...filter.price, $lte: parseFloat(query.maxPrice) };
  if (query.tags !== undefined) filter['tags.name'] = query.tags;
  if (query.startDate !== undefined)
    filter.startDate = { $gte: query.startDate };
  if (query.endDate !== undefined) filter.endDate = { $lte: query.endDate };
  ...
  if (query.level !== undefined) filter['details.level'] = query.level;
